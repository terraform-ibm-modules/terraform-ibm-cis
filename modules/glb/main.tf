...
resource "ibm_cis_global_load_balancer" "cis_glb" {
  ...
  dynamic "region_pools" {
    ...
    content {
      ...
      pool_ids = lookup(region_pools.value, "pool_names", null) != null ? [for pool in region_pools.value["pool_names"] : ibm_cis_origin_pool.origin_pool[pool].id] : lookup(region_pools.value, "pool_ids", null)
    }
  }
  dynamic "pop_pools" {
...
resource "ibm_cis_origin_pool" "origin_pool" {
  ...
  monitor            = (lookup(each.value, "health_check_name", null) != null ? ibm_cis_healthcheck.health_check[each.value["health_check_name"]].id : null)
}
