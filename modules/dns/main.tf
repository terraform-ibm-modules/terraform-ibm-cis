...
locals {
  ...
  # tflint-ignore: terraform_unused_declarations
  validate_dns_record_content = [for record in var.dns_record_set : (record.type == "SRV" || record.type == "LOC" || record.type == "CAA") ? (record.content == null ? null : tobool("The content{} block in dns_record_set should not be defined for the DNS record- ${record.name}")) : null]
}
